...
const cancelBtn = document.querySelector('.modal-buttons button[type="button"]');
...
let bookings = JSON.parse(localStorage.getItem("assetBookings")) || [];
let selectedAsset = "";
...
document.querySelectorAll(".primary").forEach((btn) => {
    btn.addEventListener("click", () => {
        selectedAsset = btn.closest(".resource-card").querySelector("h3").textContent.trim();
        openModal();
    });
});
...
categoryButtons.forEach((button) => {
    button.addEventListener("click", () => {
        ...
        const category = button.innerText.replace(/[^\w\s]/gi, "").trim().toLowerCase();
...
function saveBookings() {
    localStorage.setItem("assetBookings", JSON.stringify(bookings));
}

function createBookingObject(employee, purpose, borrow, returnDate, time, asset) {
    return {
        id: Date.now(),
        asset,
        employee,
...
bookingForm.addEventListener("submit", (e) => {
    e.preventDefault();
    const employee = bookingForm.querySelector('input[type="text"]').value.trim();
    const purpose = bookingForm.querySelector("textarea").value.trim();
    const borrow = bookingForm.querySelector('input[type="date"]').value;
    ...
    const booking = createBookingObject(employee, purpose, borrow, returnDate, time, selectedAsset);
...
function checkReminder() {
    ...
    bookings.forEach((booking) => {
        ...
        if (minutes > 0 && minutes <= 30) {
            alert(`Reminder: ${booking.asset} booking starts in ${minutes} minutes.`);
        }
    });
}
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // FIX 1: Select by class since the HTML doesn't have id="sidebar"
    const sidebar = document.querySelector('.sidebar'); 
    
    // FIX 2: Create the overlay dynamically if it's missing from the HTML
